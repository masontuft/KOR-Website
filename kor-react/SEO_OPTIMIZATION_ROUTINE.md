...
| TASK-017 | performance     | Re-audited Track 4 against current `public/images/articles/*.webp` sizes. `bike-maintenance-schedule.webp` (272 KB, TASK-002) still open. Newly found: `when-to-replace-cassette.webp` is 200 KB, the second-largest hero image, exceeding the 150 KB threshold — recompress. Checked `Article.tsx` (hero image correctly *not* lazy-loaded, it's above the fold/LCP) and `ArticlesIndex.tsx` + related-articles thumbnails in `Article.tsx` (both already use `loading="lazy"`) — no issues there. | open   | 2026-09-19 |
