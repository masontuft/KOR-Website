...
| TASK-017 | performance      | Track 4 audit (2026-09-10): re-ran `du -sh public/images/articles/*.webp` — `bike-maintenance-schedule.webp` (272 KB, TASK-002) is still over the 150 KB threshold and remains open there. Also found a second oversized hero image not previously logged: `when-to-replace-cassette.webp` is 200 KB. Lazy-loading was spot-checked in `src/components/articles/Article.tsx` and `ArticlesIndex.tsx` — both already use `loading="lazy"`, no action needed. Logged as its own row (not bundled with TASK-002) since each recompression is a separate file/PR. | open | 2026-09-10 |
