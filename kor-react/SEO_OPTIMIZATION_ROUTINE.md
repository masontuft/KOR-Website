...
| TASK-002 | performance     | `bike-maintenance-schedule.webp` is 272 KB, the largest hero image — recompress. Fixed: re-encoded at 1024px wide (source display is capped at 420px height via `object-fit: cover`, so 1024px comfortably covers it at typical device pixel ratios) and WebP quality 75, bringing it to 128 KB with no visible detail loss. `npm run build` passes. PR pending. | in_review | 2026-07-24 |
